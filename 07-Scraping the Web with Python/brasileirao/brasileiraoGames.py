...
def make_soup():
    ...
    url = "https://www.cbf.com.br/futebol-brasileiro/competicoes/campeonato-brasileiro-serie-{}#.Ww4ABXUvw5l".format(
        division
    )
...
def get_games(soup):
    lst = soup.select(
        "div.swiper-slide.active > div > ul > li > div > .clearfix > a > div > img"
    )
    return lst
...
def write_to_file(lst):
    with open("brasileiraoGames.txt", "w") as file:
        for g in range(0, len(lst) - 1, 2):
            file.write(
                "{:^30} \033[1;34mX\033[1;33m {:^30}\033[0m|\n".format(
                    lst[g].get("title"), lst[g + 1].get("title")
                )
            )
