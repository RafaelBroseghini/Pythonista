...
def diff_by_one_letter_all(word, all_words, used_words):
    result = [x for x in all_words if diff_by_one_letter(x, word) and x not in used_words]
    return result
...
def main():
    ...
    if len(word_start) > 5 or len(word_start) < 3 or len(word_end) > 5 or len(word_end) < 3:
    ...
    while not all_ladders.isEmpty() and not found:
        ...
        possible_words = diff_by_one_letter_all(current_ladder.peek(), words_to_use, words_used)
    ...
    if result_ladder.size() == 0:
        print('It is not possible to turn', word_start, 'into', word_end, 'based on the words present in the .txt document provided.')
    print(result_ladder)
