...
class Parser:
    ...
    def error(self, stateId, tokenId, prodStack, lex):
        ...
        sys.stderr.write(
            "Next Input Symbol is " + lex + " with tokenId of " + str(tokenId) + "\n\n"
        )
        ...
        while True:
            ...
            # reduce/reduce conflict.
                ...
                for item in theState.items:
                    ...
                    # and item.dotIndex == len(prod.rhs):
                    if tokenId in item.la:
                        ...
                        # push on the new state found by following the
                        # transition on the LHS of the chosen item.

                        stateId, value = prodStack.peek()
                        prodStack.push(
                            (self.states[stateId].onClassGoTo(prod.lhsId), val)
                        )
                        reduced = True
