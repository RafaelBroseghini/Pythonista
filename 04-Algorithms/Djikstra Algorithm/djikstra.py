...
def djikstra(graph, source, target):
    ...
    while visited != unvisited:
        ...
        for neighbor in graph[current]:
            ...
            # previously know distance. If True, we set distance to current
            # distance.
            if graph[current][neighbor] + distance < distances[neighbor]:
                distances[neighbor] = graph[current][neighbor] + distance
                predecessors[neighbor] = current

        # NOTE: I will refactor code and figure out a way to return false if
        # cannot reach target.
        try:
            current = get_min_neighbor(current, visited)
        except BaseException:
            return distances, predecessors
            # return "Shortest path from {} to {} is: {}".format(source,
...
def save_to_file():
    ...
    with open("graph_lookup.txt", "w+") as file:
        ...
        for v in nodes:
            for j in nodes:
                # Here we check if nodes aren't the same. Distance is 0 from
                # node to itself.
                if v != j:
